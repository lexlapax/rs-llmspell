...
use llmspell_state_persistence::{
    backup::{
        BackupConfig, BackupManager, BackupValidation, RestoreOptions, CompressionType,
        BackupResult, BackupStatus, RecoveryOrchestrator, AdvancedRecoveryOptions,
        RecoveryProgress, RecoveryState
    },
...
mod backup_recovery_tests {
    ...
    async fn create_test_state_manager_with_backup() -> (Arc<StateManager>, Arc<BackupManager>, TempDir) {
    ...
    async fn populate_test_data(state_manager: &StateManager) -> Result<(), Box<dyn std::error::Error>> {
        // Add various types of state data
        state_manager.set(StateScope::Global, "user_settings", json!({
            "theme": "dark",
            "language": "en",
            "notifications": true
        })).await?;

        state_manager.set(StateScope::Global, "session_data", json!({
            "user_id": 12345,
            "login_time": "2025-01-27T10:00:00Z",
            "permissions": ["read", "write"]
        })).await?;

        state_manager.set(StateScope::Custom("agent_1".to_string()), "config", json!({
            "model": "gpt-4",
            "temperature": 0.7,
            "max_tokens": 2000
        })).await?;

        state_manager.set(StateScope::Custom("agent_2".to_string()), "history", json!({
            "conversations": [
    ...
    async fn test_complete_backup_recovery_cycle() {
        let (state_manager, backup_manager, _temp_dir) = create_test_state_manager_with_backup().await;
        ...
        // Validate backup
        let validation = backup_manager.validate_backup(&backup_status.id).await.unwrap();
        ...
        state_manager.set(StateScope::Global, "new_data", json!({"after_backup": true})).await.unwrap();
        
        // Verify modification exists
        let modified_data = state_manager.get(StateScope::Global, "new_data").await.unwrap();
        ...
        backup_manager.restore_backup(&backup_status.id, restore_options).await.unwrap();

        // Verify original data is restored
        let user_settings = state_manager.get(StateScope::Global, "user_settings").await.unwrap();
        assert_eq!(user_settings, Some(json!({
            "theme": "dark",
            "language": "en",
            "notifications": true
        })));

        let session_data = state_manager.get(StateScope::Global, "session_data").await.unwrap();
        assert_eq!(session_data, Some(json!({
            "user_id": 12345,
            "login_time": "2025-01-27T10:00:00Z",
            "permissions": ["read", "write"]
        })));
        ...
        let new_data = state_manager.get(StateScope::Global, "new_data").await.unwrap();
    ...
    async fn test_incremental_backup_chain() {
        ...
        // Initial data
        state_manager.set(StateScope::Global, "base_data", json!({"version": 1})).await.unwrap();
        ...
        // Add more data
        state_manager.set(StateScope::Global, "incremental_data_1", json!({"version": 2})).await.unwrap();
        ...
        // Add more data
        state_manager.set(StateScope::Global, "incremental_data_2", json!({"version": 3})).await.unwrap();
        ...
        backup_manager.restore_backup(&incremental_backup_2.id, RestoreOptions::default()).await.unwrap();

        // Verify all data is restored correctly
        let base_data = state_manager.get(StateScope::Global, "base_data").await.unwrap();
        assert_eq!(base_data, Some(json!({"version": 1})));

        let inc_data_1 = state_manager.get(StateScope::Global, "incremental_data_1").await.unwrap();
        assert_eq!(inc_data_1, Some(json!({"version": 2})));

        let inc_data_2 = state_manager.get(StateScope::Global, "incremental_data_2").await.unwrap();
    ...
    async fn test_disaster_recovery_simulation() {
        let (state_manager, backup_manager, temp_dir) = create_test_state_manager_with_backup().await;

        // Populate critical application data
        populate_test_data(&state_manager).await.unwrap();
        
        // Add more critical data
        state_manager.set(StateScope::Global, "critical_config", json!({
        ...
        backup_manager.restore_backup(&disaster_backup.id, RestoreOptions {
            verify_checksums: true,
        ...
        let critical_config = state_manager.get(StateScope::Global, "critical_config").await.unwrap();
        assert_eq!(critical_config, Some(json!({
            "database_url": "postgres://localhost:5432/myapp",
            "api_keys": ["key1", "key2"],
            "feature_flags": {"new_ui": true, "beta_features": false}
        })));

        let agent_1_config = state_manager.get(StateScope::Custom("agent_1".to_string()), "config").await.unwrap();
        ...
        assert!(recovery_duration < Duration::from_secs(5), 
                "Recovery took too long: {:?}", recovery_duration);
    ...
    async fn test_backup_integrity_validation() {
        ...
        state_manager.set(StateScope::Global, "string_data", json!("test string")).await.unwrap();
        state_manager.set(StateScope::Global, "number_data", json!(42)).await.unwrap();
        state_manager.set(StateScope::Global, "boolean_data", json!(true)).await.unwrap();
        state_manager.set(StateScope::Global, "array_data", json!([1, 2, 3, 4, 5])).await.unwrap();
        state_manager.set(StateScope::Global, "object_data", json!({
        ...
        println!("Backup validation successful for backup: {}", backup_status.id);
    ...
    async fn test_partial_recovery_scenarios() {
        ...
        state_manager.set(StateScope::Global, "global_config", json!({"setting": "global"})).await.unwrap();
        state_manager.set(StateScope::Custom("service_a".to_string()), "config", json!({"service": "a"})).await.unwrap();
        state_manager.set(StateScope::Custom("service_b".to_string()), "config", json!({"service": "b"})).await.unwrap();
        state_manager.set(StateScope::Custom("service_c".to_string()), "config", json!({"service": "c"})).await.unwrap();

        // Create backup
        let backup_status = backup_manager.create_backup(false).await.unwrap();

        // Modify state after backup
        state_manager.set(StateScope::Custom("service_a".to_string()), "config", json!({"service": "a_modified"})).await.unwrap();
        state_manager.delete(StateScope::Custom("service_b".to_string()), "config").await.unwrap();
        ...
        backup_manager.restore_backup(&backup_status.id, dry_run_options).await.unwrap();

        // Verify state is unchanged after dry run
        let service_a_config = state_manager.get(StateScope::Custom("service_a".to_string()), "config").await.unwrap();
        assert_eq!(service_a_config, Some(json!({"service": "a_modified"})));

        let service_b_config = state_manager.get(StateScope::Custom("service_b".to_string()), "config").await.unwrap();
    ...
    async fn test_backup_retention_and_cleanup() {
        ...
        for i in 0..7 {
            ...
            state_manager.set(StateScope::Global, &format!("data_{}", i), json!({"value": i})).await.unwrap();
        ...
        assert!(backups_after_cleanup.len() <= 5, "Should have 5 or fewer backups after retention");
    ...
    async fn test_backup_performance_impact() {
        ...
        backup_manager.restore_backup(&backup_status.id, RestoreOptions::default()).await.unwrap();
        let restore_duration = restore_start.elapsed().unwrap();

        // Performance assertions
        assert!(backup_duration < Duration::from_secs(10), 
                "Backup creation should complete within 10 seconds, took {:?}", backup_duration);
        
        assert!(validation_duration < Duration::from_secs(5),
                "Backup validation should complete within 5 seconds, took {:?}", validation_duration);
        
        assert!(restore_duration < Duration::from_secs(10),
                "Backup restore should complete within 10 seconds, took {:?}", restore_duration);
    ...
    async fn test_concurrent_backup_operations() {
        ...
        for i in 0..3 {
            ...
            let future = tokio::spawn(async move {
                ...
                state_manager_clone.set(StateScope::Global, &format!("concurrent_{}", i), json!({"id": i})).await.unwrap();
        ...
        let backup_statuses: Vec<BackupStatus> = backup_results.unwrap()
            .into_iter()
        ...
        for backup_status in &backup_statuses {
            let validation = backup_manager.validate_backup(&backup_status.id).await.unwrap();
            assert!(validation.is_valid, "Concurrent backup {} should be valid", backup_status.id);
        }
        ...
        for (i, status) in backup_statuses.iter().enumerate() {
            println!("  Backup {}: {} ({} bytes)", i, status.id, status.size_bytes);
        }
    }
}