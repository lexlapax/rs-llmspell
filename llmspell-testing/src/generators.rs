...
use llmspell_core::{
    ...
    traits::{
        ...
        agent::{MessageRole, ConversationMessage, AgentConfig},
        tool::{ToolCategory, SecurityLevel, ToolSchema},
        workflow::{WorkflowStep, RetryPolicy, WorkflowConfig, WorkflowStatus},
    },
};
...
pub fn version_strategy() -> impl Strategy<Value = Version> {
    (0u32..100u32, 0u32..100u32, 0u32..1000u32)
...
pub fn component_metadata_strategy() -> impl Strategy<Value = ComponentMetadata> {
    (
        ...
        ".*",
        version_strategy(),
    ).prop_map(|(name, description, version)| {
...
pub fn agent_config_strategy() -> impl Strategy<Value = AgentConfig> {
    ...
    ).prop_map(|(max_conversation_length, system_prompt, temperature, max_tokens)| {
        AgentConfig {
...
pub fn retry_policy_strategy() -> impl Strategy<Value = RetryPolicy> {
    (1u32..10u32, 1u32..60u32, any::<bool>())
        .prop_map(|(max_attempts, backoff_seconds, exponential_backoff)| {
            RetryPolicy {
                max_attempts,
                backoff_seconds,
                exponential_backoff,
...
pub fn workflow_step_strategy() -> impl Strategy<Value = WorkflowStep> {
    ...
    ).prop_map(|(name, component_id, dependencies, retry_policy, timeout_secs)| {
