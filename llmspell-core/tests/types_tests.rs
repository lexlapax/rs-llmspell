...
use llmspell_core::{ComponentId, Version, ComponentMetadata};
...
fn test_component_id_thread_safety() {
    ...
    let handles: Vec<_> = (0..10)
        .map(|i| {
            thread::spawn(move || {
                ComponentId::from_name(&format!("thread-{}", i))
            })
        })
        .collect();
    
    let ids: HashSet<ComponentId> = handles
        .into_iter()
        .map(|h| h.join().unwrap())
        .collect();
...
fn test_version_display() {
    ...
    assert_eq!(v_max.to_string(), format!("{}.{}.{}", u32::MAX, u32::MAX, u32::MAX));
}
