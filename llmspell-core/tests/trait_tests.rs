...
use llmspell_core::{
    ...
    traits::{
        ...
        workflow::{Workflow, WorkflowStep, WorkflowStatus, RetryPolicy},
    },
};
...
async fn test_base_agent_mock_behavior() {
    ...
    mock.expect_execute()
        ...
        .times(1)
        .returning(|input, _| {
            Ok(AgentOutput::new(format!("Mocked: {}", input.prompt)))
        });
...
async fn test_agent_mock_conversation_management() {
    ...
    mock.add_message(ConversationMessage::system("System prompt".to_string())).await.unwrap();
...
async fn test_tool_mock_schema_validation() {
    ...
    mock.expect_validate_parameters()
        ...
        .times(1)
        .returning(|_| Err(LLMSpellError::Validation {
    ...
    assert!(mock.validate_parameters(&serde_json::json!({"input": "test"})).await.is_ok());
    assert!(mock.validate_parameters(&serde_json::json!({})).await.is_err());
}
