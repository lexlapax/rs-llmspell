...
use llmspell_core::{
    ...
    traits::{
        ...
        tool::{ToolSchema, ToolCategory, SecurityLevel, ParameterDef, ParameterType},
        workflow::{WorkflowStep, StepResult, WorkflowStatus, RetryPolicy},
    },
};
use serde_json;
...
fn test_agent_config_json_roundtrip() {
    ...
    assert_eq!(config.max_conversation_length, deserialized.max_conversation_length);
...
fn test_complex_nested_serialization() {
    ...
    let input = AgentInput::new("complex test".to_string())
        .with_context("nested".to_string(), serde_json::json!({
            "level1": {
                "level2": {
                    "level3": {
