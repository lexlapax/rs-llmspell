...
use llmspell_kernel::api::{KernelHandle, start_embedded_kernel_with_executor};
use tokio::net::TcpListener;
use serde_json::json;
use llmspell_templates::core::{Template, TemplateMetadata, TemplateCategory, TemplateParams, TemplateOutput};
use llmspell_templates::core::provider_parameters;
use llmspell_templates::validation::ConfigSchema;
use async_trait::async_trait;
use std::sync::Arc;
use tokio::sync::Mutex;
use llmspell_core::traits::script_executor::{ScriptExecutor, ScriptExecutionOutput};
use llmspell_core::LLMSpellError;
use std::collections::HashMap;
use llmspell_config::LLMSpellConfig;
use llmspell_testing::state_helpers::create_test_state_manager;
use llmspell_hooks::{HookRegistry, HookExecutor};
...
impl Template for TestTemplate {
    fn metadata(&self) -> &TemplateMetadata { &self.meta }
    fn config_schema(&self) -> ConfigSchema { 
        let mut params = vec![
            llmspell_templates::validation::ParameterSchema::optional(
                "max_tokens", "tokens", llmspell_templates::validation::ParameterType::Integer, json!(100)
            )
        ];
    ...
    async fn execute(&self, _params: TemplateParams, _ctx: llmspell_templates::context::ExecutionContext) -> Result<TemplateOutput, llmspell_templates::error::TemplateError> {
        unimplemented!()
    }
}
...
impl ScriptExecutor for DummyScriptExecutor {
    ...
    fn language(&self) -> &'static str { "dummy" }
...
async fn test_authenticated_template_launch_persistence() -> Result<()> {
    ...
    // Register test template
    {
        ...
        llmspell_templates::registry::global_registry().register(Arc::new(t)).unwrap();
    }
    ...
    tokio::spawn(async move {
        ...
        let runtime_config = Arc::new(tokio::sync::RwLock::new(llmspell_config::env::EnvRegistry::new()));
    ...
    // 3. Launch Template
    let launch_resp = client.post(format!("{}/api/templates/test-launch-v1/launch", base_url))
    ...
    let session_resp = client.get(format!("{}/api/sessions/{}", base_url, session_id))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await?;
