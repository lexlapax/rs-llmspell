...
async fn test_spa_fallback() {
    ...
    let response = app
        .oneshot(Request::builder().uri("/random-page").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_api_exclusion() {
    ...
    let response = app
        .oneshot(Request::builder().uri("/api/some-resource").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_serve_static_file() {
    ...
    let response = app
        .oneshot(Request::builder().uri("/vite.svg").body(Body::empty()).unwrap())
        .await
        .unwrap();

    assert_eq!(response.status(), StatusCode::OK);
    assert!(response.headers().get("content-type").unwrap().to_str().unwrap().contains("image/svg+xml"));
}
