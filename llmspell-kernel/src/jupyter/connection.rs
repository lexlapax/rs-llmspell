...
impl ConnectionInfo {
    ...
    /// # Errors
    ///
    /// Returns an error if port allocation fails or configuration is invalid.
    pub fn new(config: ConnectionConfig) -> Result<Self> {
        ...
        let key = config
            .custom_key
            .map_or_else(Self::generate_hmac_key, |key| key);
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let connection_info: Self = serde_json::from_str(&content).with_context(|| {
            format!(
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        let json =
            serde_json::to_string_pretty(self).context("Failed to serialize connection info")?;
    ...
    pub fn temp_connection_file(kernel_id: &str) -> PathBuf {
        let filename = format!("kernel-{kernel_id}.json");
    ...
    /// # Errors
    ///
    /// Returns an error if ports are invalid, duplicated, or configuration is unsupported.
    pub fn validate(&self) -> Result<()> {
        ...
        hex::decode(&self.key).with_context(|| format!("Invalid hex key: {}", self.key))?;
    ...
    fn allocate_ports(start_port: u16, count: usize) -> Result<Vec<u16>> {
        ...
        while ports.len() < count {
            ...
            if current_port == 0 {
                return Err(anyhow::anyhow!(
                    "Exhausted port range without finding {} available ports",
                    count
                ));
            }
        }
...
impl ConnectionConfig {
    ...
    #[must_use]
    pub const fn with_port_range(mut self, start_port: u16) -> Self {
