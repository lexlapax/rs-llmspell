...
fn bench_heavy_output_throughput(c: &mut Criterion) {
    ...
    group.bench_function("baseline_direct", |b| {
        b.iter(|| {
            ...
            for i in 0..1000 {
                let line = format!("Line {}: Some longer output text that simulates real log data\n", i);
    ...
    group.bench_function("unbuffered", |b| {
        ...
        b.iter(|| {
            for i in 0..1000 {
                let _ = io_context.stdout.write_line(
                    &format!("Line {}: Some longer output text that simulates real log data", i)
                );
            }
            let _ = io_context.stdout.flush();
            mock.lines.lock().unwrap().clear();
