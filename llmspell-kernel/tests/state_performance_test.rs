...
async fn test_state_performance_targets() {
    ...
    assert!(avg_write_ms < 5.0, "Write performance target not met: {:.3}ms > 5ms", avg_write_ms);
    assert!(avg_read_ms < 1.0, "Read performance target not met: {:.3}ms > 1ms", avg_read_ms);
