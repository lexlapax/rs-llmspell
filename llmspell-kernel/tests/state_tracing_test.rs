...
use llmspell_kernel::state::{
    ...
    persistence::{FilePersistence, MemoryPersistence, StatePersistence, KernelStateSnapshot},
...
async fn test_persistence_operations_tracing() -> Result<()> {
    ...
    tracing::info_span!("test_save_state").in_scope(|| async {
        file_persistence.save_state(&snapshot).await.unwrap();
    }).await;

    // Test load_state tracing
    tracing::info_span!("test_load_state").in_scope(|| async {
        let loaded = file_persistence.load_state().await.unwrap();
        assert!(loaded.is_some());
    }).await;

    // Test state_exists tracing
    tracing::info_span!("test_state_exists").in_scope(|| async {
        let exists = file_persistence.state_exists().await.unwrap();
        assert!(exists);
    }).await;

    // Test save_snapshot tracing
    tracing::info_span!("test_save_snapshot").in_scope(|| async {
        file_persistence.save_snapshot("test_snapshot", &snapshot).await.unwrap();
    }).await;

    // Test load_snapshot tracing
    tracing::info_span!("test_load_snapshot").in_scope(|| async {
        let loaded = file_persistence.load_snapshot("test_snapshot").await.unwrap();
        assert!(loaded.is_some());
    }).await;

    // Test list_snapshots tracing
    tracing::info_span!("test_list_snapshots").in_scope(|| async {
        let snapshots = file_persistence.list_snapshots().await.unwrap();
        assert_eq!(snapshots.len(), 1);
    }).await;

    // Test delete_state tracing
    tracing::info_span!("test_delete_state").in_scope(|| async {
...
async fn test_memory_persistence_tracing() -> Result<()> {
    ...
    tracing::info_span!("memory_persistence_ops").in_scope(|| async {
        ...
        memory_persistence.save_snapshot("snapshot1", &snapshot).await.unwrap();
        memory_persistence.save_snapshot("snapshot2", &snapshot).await.unwrap();
...
async fn test_state_manager_tracing() -> Result<()> {
    ...
    tracing::info_span!("test_set_operations").in_scope(|| async {
        state_manager.set(
            StateScope::Global,
            "test_key",
            json!("test_value")
    ...
    tracing::info_span!("test_get_operations").in_scope(|| async {
        let value = state_manager.get(StateScope::Global, "test_key").await.unwrap();
        assert!(value.is_some());

        let value = state_manager.get_with_class(
            StateScope::Agent("agent1".to_string()),
            "agent_key",
            None
        ).await.unwrap();
    ...
    tracing::info_span!("test_delete_operations").in_scope(|| async {
        let deleted = state_manager.delete(StateScope::Global, "test_key").await.unwrap();
        assert!(deleted);
    }).await;

    // Test list_keys tracing
    tracing::info_span!("test_list_keys").in_scope(|| async {
        state_manager.set(StateScope::Global, "key1", json!("value1")).await.unwrap();
        state_manager.set(StateScope::Global, "key2", json!("value2")).await.unwrap();
    ...
    tracing::info_span!("test_clear_scope").in_scope(|| async {
...
async fn test_agent_state_operations_tracing() -> Result<()> {
    ...
    use llmspell_kernel::state::agent_state::{AgentStateData, AgentMetadata, ExecutionState, ToolUsageStats};
    ...
    tracing::info_span!("test_save_agent_state").in_scope(|| async {
        state_manager.save_agent_state(&agent_state).await.unwrap();
    }).await;

    // Test load_agent_state tracing
    tracing::info_span!("test_load_agent_state").in_scope(|| async {
        let loaded = state_manager.load_agent_state("test_agent").await.unwrap();
        assert!(loaded.is_some());
    }).await;

    // Test load_agent_state_fast tracing
    tracing::info_span!("test_load_agent_state_fast").in_scope(|| async {
        let loaded = state_manager.load_agent_state_fast("test_agent").await.unwrap();
        assert!(loaded.is_some());
    }).await;

    // Test get_agent_metadata tracing
    tracing::info_span!("test_get_agent_metadata").in_scope(|| async {
        let metadata = state_manager.get_agent_metadata("test_agent").await.unwrap();
        assert!(metadata.is_some());
    }).await;

    // Test list_agent_states tracing
    tracing::info_span!("test_list_agent_states").in_scope(|| async {
    ...
    tracing::info_span!("test_delete_agent_state").in_scope(|| async {
        let deleted = state_manager.delete_agent_state("test_agent").await.unwrap();
        assert!(deleted);
    }).await;
...
async fn test_scoped_operations_tracing() -> Result<()> {
    ...
    tracing::info_span!("test_scoped_ops").in_scope(|| async {
        state_manager.set_scoped(
            scope.clone(),
            "scoped_key",
            json!("scoped_value")
        ).await.unwrap();

        let value = state_manager.get_scoped(scope.clone(), "scoped_key").await.unwrap();
        assert!(value.is_some());

        let deleted = state_manager.delete_scoped(scope.clone(), "scoped_key").await.unwrap();
        assert!(deleted);
    }).await;
...
async fn test_hook_persistence_tracing() -> Result<()> {
    ...
    tracing::info_span!("test_hook_persistence").in_scope(|| async {
