...
fn test_performance_improvement() {
    ...
    // Note: Casting u128 to f64 loses precision, but for comparing performance ratios
    // the loss is acceptable as we only need approximate values
    ...
    println!("Improvement ratio: {improvement_ratio:.2}x");
...
fn test_batch_write_efficiency() {
    ...
    assert!(!lines.is_empty(), "Batch write should have occurred");
...
fn test_concurrent_access() {
    ...
    for thread_id in 0..5 {
        ...
        let handle = thread::spawn(move || {
            for i in 0..20 {
                buffered_clone
                    .write_line(&format!("Thread {thread_id} Line {i}"))
                    .unwrap();
            }
        });
        handles.push(handle);
    ...
    for thread_id in 0..5 {
        for i in 0..20 {
            assert!(
                content.contains(&format!("Thread {thread_id} Line {i}")),
                "Missing line from thread {thread_id}, line {i}"
            );
        }
    }
}
