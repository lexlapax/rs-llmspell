...
use llmspell_utils::time::{
    ...
    subtract_duration, weekday_name, DATE_FORMATS,
};
    ...
    async fn process_operation(&self, params: &Value) -> Result<Value> {
        ...
        match operation {
            "parse" => {
                let input = params
                    ...
                    .ok_or_else(|| LLMSpellError::Validation {
                        message: "Missing 'input' parameter for parse operation".to_string(),
                        field: Some("input".to_string()),
                    })?;

                let dt = parse_datetime(input).map_err(|e| LLMSpellError::Tool {
            ...
            "add" | "subtract" => {
                ...
                let unit = params.get("unit").and_then(|v| v.as_str()).ok_or_else(|| {
                    LLMSpellError::Validation {
                        message: "Missing 'unit' parameter".to_string(),
                ...
                .map_err(|e| LLMSpellError::Tool {
                    message: format!("Failed to {} duration: {}", operation, e),
                    tool_name: Some(self.metadata.name.clone()),
                    source: None,
                })?;
...
mod tests {
    ...
    async fn test_parse_operation() {
        ...
        assert_eq!(output["parsed"]["month"], 1);
        assert_eq!(output["parsed"]["day"], 15);
        assert_eq!(output["parsed"]["hour"], 10);
        assert_eq!(output["parsed"]["minute"], 30);
        assert_eq!(output["parsed"]["weekday"], "Monday");
    }
    ...
    async fn test_timezone_conversion() {
        ...
        assert_eq!(output["operation"], "convert_timezone");
        assert_eq!(output["target_timezone"], "America/New_York");
    ...
    async fn test_date_arithmetic() {
        ...
        assert_eq!(output["operation"], "subtract");
        assert!(output["result"]
            .as_str()
            .unwrap()
    ...
    async fn test_date_info() {
        ...
        assert_eq!(output["operation"], "info");
