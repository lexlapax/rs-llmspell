...
impl ProviderConfig {
    ...
    pub fn from_env(name: &str) -> Result<Self, LLMSpellError> {
        ...
        let model = std::env::var(format!("{}MODEL", env_prefix))
...
pub type ProviderFactory = Box<dyn Fn(ProviderConfig) -> Result<Box<dyn ProviderInstance>, LLMSpellError> + Send + Sync>;
...
impl ProviderRegistry {
    ...
    where
        F: Fn(ProviderConfig) -> Result<Box<dyn ProviderInstance>, LLMSpellError> + Send + Sync + 'static,
    {
        self.factories.insert(name.into(), Box::new(factory));
    }
    
    /// Create a provider instance
    pub fn create(&self, config: ProviderConfig) -> Result<Box<dyn ProviderInstance>, LLMSpellError> {
        let factory = self.factories.get(&config.name)
...
impl ProviderManager {
    ...
    pub async fn query_capabilities(&self, name: Option<&str>) -> Result<ProviderCapabilities, LLMSpellError> {
        let instances = self.instances.read().await;
        let default = self.default_provider.read().await;
...
mod tests {
    ...
    async fn test_provider_manager_initialization() {
        ...
        manager.register_provider("mock", |_config| {
            Err(LLMSpellError::Provider {
                message: "Mock provider".to_string(),
                provider: Some("mock".to_string()),
                source: None,
