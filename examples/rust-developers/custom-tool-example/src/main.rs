...
impl TextAnalyzerTool {
    fn new() -> Self {
        Self {
            metadata: ComponentMetadata::new("text_analyzer".to_string(), "Analyzes text for word count, character count, and sentiment".to_string()),
        }
    }
    ...
    fn analyze_text(&self, text: &str, operation: &str) -> Value {
        match operation {
            ...
            "sentiment" => {
                ...
                let pos_count = positive_words.iter().filter(|word| text_lower.contains(*word)).count();
                let neg_count = negative_words.iter().filter(|word| text_lower.contains(*word)).count();
...
impl BaseAgent for TextAnalyzerTool {
    ...
    async fn validate_input(&self, input: &AgentInput) -> Result<(), LLMSpellError> {
        if let Some(params) = input.parameters.get("parameters") {
            if !params.get("text").and_then(|v| v.as_str()).map_or(false, |s| !s.is_empty()) {
...
impl Tool for TextAnalyzerTool {
    ...
    fn schema(&self) -> ToolSchema {
        ...
        .with_parameter(ParameterDef {
            ...
            param_type: ParameterType::String,
            description: "Analysis operation: word_count, char_count, line_count, sentiment".to_string(),
            required: false,
...
impl MathCalculatorTool {
    fn new() -> Self {
        Self {
            metadata: ComponentMetadata::new("math_calculator".to_string(), "Performs basic mathematical operations".to_string()),
        }
    }
...
impl BaseAgent for MathCalculatorTool {
    ...
    async fn execute_impl(
        ...
        info!("Performing calculation: {} with a={}, b={:?}", operation, a, b);
    ...
    async fn validate_input(&self, input: &AgentInput) -> Result<(), LLMSpellError> {
        if let Some(params) = input.parameters.get("parameters") {
            if !params.get("operation").and_then(|v| v.as_str()).map_or(false, |s| !s.is_empty()) {
                return Err(LLMSpellError::Validation {
...
async fn main() -> Result<()> {
    ...
    println!("   ✅ Created TextAnalyzerTool: {}", text_analyzer.metadata().name);
    println!("   ✅ Created MathCalculatorTool: {}", math_calculator.metadata().name);
    println!("   📊 TextAnalyzer - Category: {:?}, Security: {:?}", 
             text_analyzer.category(), text_analyzer.security_level());
    println!("   🔢 MathCalculator - Category: {:?}, Security: {:?}", 
             math_calculator.category(), math_calculator.security_level());
    ...
    // Test word count
    let word_count_input = AgentInput::text("analyze")
        .with_parameter("parameters", json!({
            "text": "Count the words in this sentence please.",
            "operation": "word_count"
        }));
    
    match text_analyzer.execute_impl(word_count_input, ExecutionContext::default()).await {
        Ok(output) => {
    ...
    for (operation, a, b) in math_tests {
        ...
        let math_input = AgentInput::text("calculate")
            .with_parameter("parameters", params);
        
        match math_calculator.execute_impl(math_input, ExecutionContext::default()).await {
            Ok(output) => {
    ...
    println!("   📄 MathCalculator schema: {}", math_calculator.schema().name);
