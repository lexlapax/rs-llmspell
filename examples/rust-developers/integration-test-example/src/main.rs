...
impl IntegrationScenario {
    ...
    pub async fn execute_tool(&self, tool_name: &str, input: AgentInput) -> Result<AgentOutput, LLMSpellError> {
    ...
    pub async fn execute_agent(&self, agent_name: &str, input: AgentInput) -> Result<AgentOutput, LLMSpellError> {
...
async fn demonstrate_unit_testing() -> Result<()> {
    ...
    let input2 = AgentInput::text("")
        .with_parameter("input", json!("Parameter input test"));
    let context = ExecutionContext::new();
    ...
    println!("   ✅ Call log verification: {} calls recorded", calls.len());
...
async fn demonstrate_mock_testing() -> Result<()> {
    ...
    println!("   ✅ Mock agent call count: {}", mock_agent.get_call_count());
...
async fn demonstrate_integration_testing() -> Result<()> {
    ...
    // Test error handling
    match scenario.execute_tool("nonexistent", AgentInput::text("test")).await {
...
async fn demonstrate_test_fixtures() -> Result<()> {
    ...
    let tool_result = fixture.echo_tool.execute_impl(input.clone(), context.clone()).await?;
    let agent_result = fixture.mock_agent.execute_impl(input, context).await?;
    
    println!("   ✅ Fixture tool result: length = {} chars", tool_result.text.len());
    println!("   ✅ Fixture agent result: {}", agent_result.text);
    
    // Show state tracking
    println!("   ✅ Tool calls: {}", fixture.echo_tool.get_call_log().len());
    println!("   ✅ Agent calls: {}", fixture.mock_agent.get_call_count());
    
    // Reset fixture state
    fixture.reset();
    println!("   ✅ Fixture reset - tool calls: {}", fixture.echo_tool.get_call_log().len());
...
async fn demonstrate_schema_validation() -> Result<()> {
    ...
    let valid_input = AgentInput::text("test")
        .with_parameter("input", json!("valid parameter"));
...
mod tests {
    ...
    async fn test_echo_tool_parameter_input() {
        ...
        let input = AgentInput::text("")
            .with_parameter("input", json!("param test"));
        let context = ExecutionContext::new();
    ...
    async fn test_test_fixture() {
        ...
        let input = AgentInput::text("fixture test");
        let context = ExecutionContext::new();
        let _ = fixture.echo_tool.execute_impl(input.clone(), context.clone()).await.unwrap();
        let _ = fixture.mock_agent.execute_impl(input, context).await.unwrap();
