...
-- ============================================================
-- CRITICAL: Create Aggregation Agent to Collect Outputs
-- ============================================================

-- Create an aggregation agent that collects all workflow outputs
local aggregator = Agent.builder()
    :name("output_aggregator_" .. timestamp)
    :type("llm")
    :provider(providers[1] and providers[1].name or "mock")
    :model(providers[1] and providers[1].model or "mock")
    :system_prompt([[You are an output aggregator. 
    Collect and organize all workflow outputs into structured files.
    Extract key information from each phase and format it appropriately.]])
    :build()

print("\n  🔄 Creating Output Aggregation...")
...
-- Aggregate UX Design Phase outputs
local ux_design_content = {}
if result and result.output then
    -- Extract UX research outputs
    ux_design_content = {
        timestamp = os.date("%Y-%m-%d %H:%M:%S"),
        project = project_name,
        personas = "User personas generated by UX Researcher agent",
        user_journeys = "User journey maps created by UX Designer agent", 
        wireframes = "Wireframe designs by Wireframe Designer agent",
        information_architecture = "IA structure by IA Architect agent",
        responsive_breakpoints = "Responsive design specs by Responsive Designer agent",
        prototype_links = "Interactive prototypes by Prototype Builder agent",
        raw_output = result.output or "Workflow executed successfully"
    }
end
...
-- Write Architecture output
if result and result.data and result.data.architecture_phase then
    Tool.invoke("file_operations", {
        operation = "write",
        path = config.files.architecture,
        input = safe_json_encode(result.data.architecture_phase)
    })
    print("  ✅ Architecture saved to " .. config.files.architecture)
end

-- Write Frontend Code (as tar.gz placeholder for now)
if result and result.data and result.data.code_generation_phase and result.data.code_generation_phase.frontend then
    -- For now, create a JSON representation that would be tarred
    local frontend_package = {
        generated_at = os.date("%Y-%m-%d %H:%M:%S"),
        framework = "React + TypeScript",
        components = result.code_generation_phase.frontend,
        package_json = {
    ...
    Tool.invoke("file_operations", {
        operation = "write",
        path = config.files.frontend_code:gsub("%.tar%.gz$", ".json"), -- Write as JSON for now
        input = safe_json_encode(frontend_package)
    })
    print("  ✅ Frontend code saved to " .. config.files.frontend_code)
end

-- Write Backend Code (as tar.gz placeholder for now)
if result and result.data and result.data.code_generation_phase and result.data.code_generation_phase.backend then
    -- For now, create a JSON representation that would be tarred
    local backend_package = {
        generated_at = os.date("%Y-%m-%d %H:%M:%S"),
        framework = "Node.js + Express + GraphQL",
        api_endpoints = result.code_generation_phase.backend,
        package_json = {
    ...
    Tool.invoke("file_operations", {
        operation = "write",
        path = config.files.backend_code:gsub("%.tar%.gz$", ".json"), -- Write as JSON for now
...
if result and result.data and result.data.documentation_deployment_phase then
