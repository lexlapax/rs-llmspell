...
-- ============================================================
-- State-Based Output Collection (NEW in 7.3.8)
-- ============================================================
...
if result and result.success then
    -- UX Design Phase outputs
    local ux_design_output = main_controller:get_output("ux_design_phase")
    local ux_design_content = {}
    
    if ux_design_output then
        -- Extract UX research outputs from state
        ux_design_content = {
            timestamp = os.date("%Y-%m-%d %H:%M:%S"),
            project = project_name,
            personas = ux_design_output.personas or "User personas generated by UX Researcher agent",
            user_journeys = ux_design_output.user_journeys or "User journey maps created by UX Designer agent",
            wireframes = ux_design_output.wireframes or "Wireframe designs by Wireframe Designer agent",
            information_architecture = ux_design_output.ia or "IA structure by IA Architect agent",
            responsive_breakpoints = ux_design_output.breakpoints or "Responsive design specs by Responsive Designer agent",
            prototype_links = ux_design_output.prototype or "Interactive prototypes by Prototype Builder agent",
            state_retrieved = true  -- Indicate this was retrieved from state
        }
    else
        -- Fallback if state access fails
        ux_design_content = {
            timestamp = os.date("%Y-%m-%d %H:%M:%S"),
            project = project_name,
            message = "UX Design phase completed - outputs stored in state",
    ...
    -- Architecture output using state
    local architecture_output = main_controller:get_output("architecture_phase")
    if architecture_output then
        Tool.invoke("file_operations", {
            operation = "write",
            path = config.files.architecture,
            input = safe_json_encode(architecture_output)
        })
        print("  ✅ Architecture saved to " .. config.files.architecture)
    end
    
    -- Frontend Code using state (with direct State.get() as alternative method)
    local frontend_output = nil
    if result.execution_id then
        -- Try direct State access for demonstration
        frontend_output = State.get("workflow:" .. result.execution_id .. ":code_generation_phase:frontend")
    end
    if not frontend_output then
        -- Fallback to helper method
        local code_gen_output = main_controller:get_output("code_generation_phase")
        frontend_output = code_gen_output and code_gen_output.frontend
    end
