...
pub async fn handle_config_command(
    ...
    match command {
        ...
        ConfigCommands::Validate { config } => validate_config(config, output_format).await,
        ConfigCommands::Show { section, format } => {
            show_config(section, format, output_format).await
        }
    }
}
...
async fn show_config(
    ...
    let config_data = if let Some(ref section_name) = section {
        ...
        match section_name.as_str() {
            ...
            "debug" => serde_json::to_value(&runtime_config.debug)?,
            ...
            "hooks" => serde_json::to_value(&runtime_config.hooks)?,
            "events" => serde_json::to_value(&runtime_config.events)?,
            _ => {
                anyhow::bail!(
                    "Unknown config section '{}'. Available: rag, debug, providers, engines, runtime, tools, hooks, events",
                    section_name
                );
            }
        }
