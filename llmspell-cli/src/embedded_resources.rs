...
pub static EMBEDDED_APPS: Lazy<HashMap<&'static str, EmbeddedApp>> = Lazy::new(|| {
    ...
    apps.insert("file-organizer", EmbeddedApp {
        name: "file-organizer",
    ...
    apps.insert("research-collector", EmbeddedApp {
    ...
    apps.insert("content-creator", EmbeddedApp {
    ...
    apps.insert("communication-manager", EmbeddedApp {
    ...
    apps.insert("process-orchestrator", EmbeddedApp {
    ...
    apps.insert("code-review-assistant", EmbeddedApp {
    ...
    apps.insert("webapp-creator", EmbeddedApp {
        name: "webapp-creator",
...
pub fn extract_app(app_name: &str) -> anyhow::Result<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    let temp_dir = std::env::temp_dir().join(format!("llmspell-{}-{}", app_name, uuid::Uuid::new_v4()));
