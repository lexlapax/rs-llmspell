...
impl ComponentRegistry {
    ...
    /// Register a workflow
    pub fn register_workflow(&self, name: String, workflow: Arc<dyn Workflow>) -> Result<(), LLMSpellError> {
...
mod tests {
    ...
    impl MockAgent {
        fn new() -> Self {
            Self {
                metadata: llmspell_core::ComponentMetadata::new("mock".to_string(), "Mock agent".to_string()),
    ...
    impl Agent for MockAgent {
        ...
        async fn get_conversation(&self) -> Result<Vec<llmspell_core::traits::agent::ConversationMessage>, LLMSpellError> {
            Ok(vec![])
        }
        
        async fn add_message(&mut self, _message: llmspell_core::traits::agent::ConversationMessage) -> Result<(), LLMSpellError> {
            Ok(())
        }
