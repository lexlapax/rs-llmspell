...
use mlua::{Lua, Result as LuaResult, Table, Function, Value, TableExt, UserData, UserDataMethods, AnyUserDataExt};
...
pub fn inject_streaming_api(
    ...
    let create_stream_fn = lua.create_function(|lua, f: Function| -> LuaResult<Table> {
        ...
        // Add next() method
        stream.set("next", lua.create_function(|_lua, stream: Table| -> LuaResult<Value> {
        ...
        // Add isDone() method
        stream.set("isDone", lua.create_function(|_lua, stream: Table| -> LuaResult<bool> {
            stream.get("_done")
        })?)?;
        
        // Add collect() method to get all values
        stream.set("collect", lua.create_function(|lua, stream: Table| -> LuaResult<Table> {
    ...
    streaming_table.set("create", create_stream_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set streaming.create: {}", e),
        source: None,
    })?;
    
    // Create yield helper for use inside coroutines
    let yield_fn = lua.create_function(|_lua, value: Value| -> LuaResult<()> {
    ...
    streaming_table.set("yield", yield_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set streaming.yield: {}", e),
        source: None,
    })?;
    
    // Set the streaming table as a global
    lua.globals().set("Streaming", streaming_table).map_err(|e| LLMSpellError::Component {
...
struct StreamReceiver {
    receiver: Arc<parking_lot::Mutex<mpsc::Receiver<String>>>,
}
...
impl UserData for StreamReceiver {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_async_method("next", |lua, this, ()| async move {
            let mut receiver = this.receiver.lock();
        ...
        methods.add_method("is_closed", |_lua, this, ()| {
            let receiver = this.receiver.lock();
...
pub fn create_lua_stream_bridge(
    ...
    stream.set("next", lua.create_async_function(|_lua, stream: Table| async move {
        let done: bool = stream.get("_done")?;
        if done {
            return Ok(Value::Nil);
        }
        ...
        let result = receiver_ud.call_async_method::<_, Value>("next", ()).await?;
    ...
    stream.set("collect", lua.create_async_function(|lua, stream: Table| async move {
        let results = lua.create_table()?;
        let mut idx = 1;
