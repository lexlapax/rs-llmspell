...
pub fn inject_workflow_api(
    ...
    let sequential_fn = lua.create_function(|lua, steps: mlua::Table| -> mlua::Result<mlua::Table> {
        let workflow = lua.create_table()?;
        workflow.set("type", "sequential")?;
        workflow.set("steps", steps)?;
        
        // Add execute method
        workflow.set("execute", lua.create_async_function(|lua, _args: mlua::Table| async move {
    ...
    let parallel_fn = lua.create_function(|lua, steps: mlua::Table| -> mlua::Result<mlua::Table> {
        let workflow = lua.create_table()?;
    ...
    workflow_table.set(&api_def.constructors.sequential[..], sequential_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Workflow.sequential: {}", e),
        source: None,
    })?;
    
    workflow_table.set(&api_def.constructors.parallel[..], parallel_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Workflow.parallel: {}", e),
        source: None,
    })?;
    
    // Add placeholder for conditional and loop
    let placeholder = lua.create_function(|_lua, _: mlua::Value| -> mlua::Result<mlua::Table> {
        Err(mlua::Error::RuntimeError("Workflow type not yet implemented".to_string()))
    }).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to create placeholder: {}", e),
        source: None,
    })?;
    
    workflow_table.set(&api_def.constructors.conditional[..], placeholder.clone()).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Workflow.conditional: {}", e),
        source: None,
    })?;
    
    workflow_table.set(&api_def.constructors.loop_workflow[..], placeholder).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Workflow.loop: {}", e),
        source: None,
    })?;
    
    // Set the Workflow table as a global
    lua.globals().set(&api_def.global_name[..], workflow_table).map_err(|e| LLMSpellError::Component {
