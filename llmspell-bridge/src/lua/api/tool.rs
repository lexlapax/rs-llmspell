...
pub fn inject_tool_api(
    ...
    let get_fn = lua.create_function(move |lua, tool_name: String| -> mlua::Result<mlua::Table> {
        ...
        tool.set("execute", lua.create_async_function(move |lua, _args: mlua::Table| {
        ...
        tool.set("getSchema", lua.create_function(|lua, _: ()| {
    ...
    let list_fn = lua.create_function(move |lua, _: ()| -> mlua::Result<mlua::Table> {
    ...
    tool_table.set(&api_def.get_function[..], get_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Tool.get: {}", e),
        source: None,
    })?;
    
    tool_table.set(&api_def.list_function[..], list_fn).map_err(|e| LLMSpellError::Component {
        message: format!("Failed to set Tool.list: {}", e),
        source: None,
    })?;
    
    // Set the Tool table as a global
    lua.globals().set(&api_def.global_name[..], tool_table).map_err(|e| LLMSpellError::Component {
