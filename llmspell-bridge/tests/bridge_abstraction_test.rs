...
use llmspell_bridge::{
    engine::{
        bridge::{ScriptEngineBridge, EngineFeatures, ExecutionContext, SecurityContext},
        factory::{EngineFactory, LuaConfig, StdlibLevel, EngineInfo},
    },
    ComponentRegistry,
...
async fn test_api_injection() {
    ...
    assert_eq!(output.output.as_bool(), Some(true), "Agent API should be injected");
}
...
async fn test_output_format_consistency() {
    ...
    for (script, expected) in test_cases {
        let output = engine.execute_script(script).await.unwrap();
        assert_eq!(output.output, expected, "Output mismatch for script: {}", script);
    }
}
...
async fn test_output_metadata() {
    ...
    // Execution time might be 0 for very fast scripts
    assert!(output.metadata.execution_time_ms >= 0);
...
fn assert_engine_compliance(engine: &dyn ScriptEngineBridge) {
    ...
    if features.streaming {
        assert!(engine.supports_streaming(), "Feature flag mismatch for streaming");
    }
}
...
async fn test_streaming_execution_stub() {
    ...
    assert!(stream_result.is_err(), "Streaming is not fully implemented yet");
}