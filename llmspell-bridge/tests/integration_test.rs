...
async fn test_script_execution_through_bridge() {
    ...
    assert!(result.get("version").unwrap().as_str().unwrap().contains("Lua"));
...
async fn test_engine_switching_integration() {
    ...
    assert!(engines.iter().any(|e| e.name == "lua"), "Lua should be in available engines");
}
...
async fn test_streaming_through_bridge() {
    ...
    let result = engine.execute_script_streaming("return 'streaming test'").await;
...
async fn test_provider_integration() {
    ...
    provider_config.providers.insert("test-openai".to_string(), ProviderConfig {
    ...
    let providers = Arc::new(
        match ProviderManager::new(provider_config).await {
            Ok(manager) => manager,
            Err(_) => {
                // If provider creation fails (no API key), create empty manager
                ProviderManager::new(ProviderManagerConfig::default()).await.unwrap()
            }
        }
    );
...
async fn test_multimodal_types_access() {
    ...
    assert!(engine.supports_multimodal(), "Lua should support multimodal");
    ...
    assert_eq!(result.get("workflow_available").unwrap().as_bool(), Some(true));
}
...
async fn test_execution_context_integration() {
    ...
    assert_eq!(retrieved.environment.get("TEST_VAR"), Some(&"test_value".to_string()));
}
...
async fn test_component_registration_integration() {
    use llmspell_core::{ComponentMetadata, BaseAgent};
    ...
    impl BaseAgent for MockAgent {
        ...
        async fn execute(&self, _input: AgentInput, _context: ExecutionContext) -> Result<AgentOutput, LLMSpellError> {
    ...
    impl Agent for MockAgent {
        ...
        async fn add_message(&mut self, _message: ConversationMessage) -> Result<(), LLMSpellError> {
            Ok(())
        }
    ...
    registry.register_agent("mock-agent".to_string(), mock_agent).unwrap();
