...
**Design Document**: `/docs/in-progress/phase-13b-design-doc.md` (2,673 lines)

### Timeline Overview

**6 Weeks** (30 working days) - detailed breakdown in design doc:
- **Week 1**: Linux CI, PostgreSQL infrastructure, VectorChord integration
- **Week 2**: Multi-tenancy (RLS), bi-temporal graph storage (native CTEs)
- **Week 3**: Agent/Workflow/Procedural state migration
- **Week 4**: Sessions, artifacts, large objects
- **Week 5**: Events, hooks, partitioning strategies
- **Week 6**: Security (API keys, secrets), integration testing, migration tooling, documentation

### Key Deliverables

**Infrastructure**:
- Linux CI support (GitHub Actions matrix: macOS + Linux)
- PostgreSQL 18 with VectorChord extension (Docker Compose)
- Connection pooling (deadpool-postgres, 20+ connections)
- Migration framework (refinery) with 12+ migration scripts

**Storage Backends** (10 components migrated):
1. **Episodic Memory**: VectorChord HNSW (768-3072 dim, 5x faster than pgvector)
2. **Semantic Memory**: Native PostgreSQL bi-temporal CTEs (not Apache AGE)
3. **Procedural Memory**: Pattern storage in JSONB
4. **RAG Documents**: VectorChord + metadata tables
5. **Agent State**: Versioned state with JSONB
6. **Workflow State**: Execution history + checkpoints
7. **Sessions**: TTL-based cleanup, large objects for artifacts
8. **Events**: Partitioned tables (monthly), correlation IDs
9. **Hooks**: Replay-enabled history with execution logs
10. **API Keys**: Encrypted storage (pgcrypto)

**Multi-Tenancy**:
- Row-Level Security (RLS) for database-enforced isolation
- Per-tenant connection pools (DashMap pattern from Phase 13)
- Session variable: `SET app.current_tenant_id = ?`
- <5% performance overhead (benchmarked)

**Migration Tooling**:
- 10 migration paths (HNSW→PostgreSQL, SurrealDB→PostgreSQL, Sled→PostgreSQL, etc.)
- Dry-run mode with validation
- Rollback support
- Data integrity checks

**Documentation**:
- PostgreSQL schema reference (12+ tables)
- Migration guide (existing backends → PostgreSQL)
- Operations guide (backup, restore, monitoring, archival)
...
- [ ] CI parallel matrix builds <10 minutes

**PostgreSQL Backend**:
- [ ] All 10 storage components have PostgreSQL implementation
- [ ] VectorChord integration functional (5x faster than pgvector)
- [ ] Bi-temporal graph queries <10ms (recursive CTEs)
- [ ] RLS overhead <5% (tenant isolation)
- [ ] Migration tooling 100% success rate on test data

**Zero Breaking Changes**:
- [ ] Existing backends (HNSW, SurrealDB, Sled) unchanged
- [ ] PostgreSQL opt-in via config (backend = "postgres")
- [ ] All Phase 13 tests passing with existing backends
- [ ] No API changes required

**Quality**:
- [ ] 100+ new tests for PostgreSQL backends
- [ ] Zero clippy warnings
- [ ] All docs updated (schema, migration, operations)
- [ ] Docker Compose tested on macOS + Linux

### Key Metrics

**Performance** (PostgreSQL vs existing backends):
| Operation | HNSW Files | PostgreSQL VectorChord | Improvement |
|-----------|------------|------------------------|-------------|
| Vector insert | 246/sec | 1,565/sec | 6.3x faster |
| Similarity search | 50ms P95 | 35ms P95 (recall@95%) | 1.4x faster |
| Graph traversal | 15ms | 8ms (bi-temporal CTE) | 1.9x faster |

**Costs** (100M vectors, AWS pricing):
- pgvector: $6,580/month (r6g.16xlarge)
- VectorChord: $247/month (r6g.xlarge) - **26.6x reduction**

**Storage Consolidation**:
- Before: 3+ systems (HNSW files, SurrealDB, Sled)
- After: Single PostgreSQL (optional)
- Backup complexity: 3 procedures → 1 (pg_dump)

### Risk Assessment

**High Risks**:
- **VectorChord Installation**: New extension, setup friction
  - Mitigation: Docker image, pgvector fallback, detailed docs
- **Bi-Temporal Query Performance**: CTEs may degrade at scale
  - Mitigation: Indexing, materialized views, early benchmarking

**Medium Risks**:
- **PostgreSQL Learning Curve**: Team expertise gaps
  - Mitigation: Comprehensive docs, examples, training
- **Migration Data Loss**: Sled → PostgreSQL failures
  - Mitigation: Validation scripts, dry-run, rollback, backups

**Low Risks**:
- Docker Compose complexity
- Large object API unfamiliarity

**Overall**: Medium risk, high value (production infrastructure)
...
**Dependencies**: Phase 13b (Linux + macOS support) ✅

**Strategic Rationale**: Phase 13b established Linux + macOS support with platform-specific GPU handling (Metal macOS, CUDA Linux, CPU both). Phase 18 extends to Windows, completing the cross-platform trifecta for production deployments.

**Components**:
- **Windows CI Support**: GitHub Actions matrix (windows-latest alongside macOS + Linux)
- **Windows-Specific GPU**: DirectX/DirectML for Candle backend (parallel to Metal/CUDA)
- **Platform Abstraction**: Unified path handling, process management, environment variables
- **Build System**: Cross-compilation support, Windows packaging (MSI installer)
- **File System**: Windows path separators, permissions, symlinks
- **Networking**: Windows socket handling, firewall considerations
- **Dependencies**: Windows-compatible versions of all dependencies

**Success Criteria**:
- [ ] `cargo build --workspace --all-targets` passes on Windows
- [ ] All 149+ tests pass on Windows environment
- [ ] GPU detection working (DirectML Windows, Metal macOS, CUDA Linux, CPU all)
- [ ] CI parallel matrix builds (macOS + Linux + Windows) <15 minutes
- [ ] Zero platform-specific compilation errors or warnings
- [ ] Windows packaging working (MSI installer or cargo install)
- [ ] File paths, permissions, and environment variables work cross-platform

**Testing Requirements**:
- Windows CI integration (GitHub Actions windows-latest)
- Cross-platform path handling tests
- GPU detection tests (DirectML on Windows)
- File system operations (permissions, symlinks, path separators)
- Network operations (socket handling, DNS resolution)
- Process spawning and management
- Environment variable handling
- Windows-specific regression tests

**Platform Matrix** (after Phase 18):
| Platform | GPU Support | CI Status | Package Format |
|----------|-------------|-----------|----------------|
| macOS | Metal (Phase 13b) | ✅ | cargo, brew |
| Linux | CUDA/CPU (Phase 13b) | ✅ | cargo, deb/rpm |
| Windows | DirectML (Phase 18) | ✅ | cargo, MSI |
