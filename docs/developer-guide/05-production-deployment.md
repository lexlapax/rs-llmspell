...
**Security, performance, scaling, monitoring, and operational excellence**

**Quick Navigation**: [Security](#part-1-security--multi-tenancy) | [Performance](#part-2-performance--scaling) | [Deployment](#part-3-deployment--operations) | [Monitoring](#part-4-monitoring--observability) | [Performance Tuning](#part-5-performance-tuning)

---

## Overview

This guide covers production deployment for rs-llmspell:
...
- System-level performance tuning (Phase 10)
...
## PART 5: Performance Tuning (Phase 10+)
...
|-----------|--------|--------|----------|--------|
| Kernel Boot | Startup time | <100ms | 85ms | ✅ Exceeds |
| Tool Invoke (CLI) | End-to-end | <50ms | 42ms | ✅ Exceeds |
| Message Protocol | Throughput | 1K msg/sec | 2.3K msg/sec | ✅ Exceeds |
| Jupyter Wire | Latency (p95) | <10ms | 8ms | ✅ Exceeds |
| DAP Protocol | Message handling | <5ms | 3ms | ✅ Exceeds |
...
**Recommendations**:
- **Multi-core systems** (4+ cores): Use default (auto-detect)
- **High-concurrency** (100+ concurrent ops): Increase `max_blocking_threads` to 1024
- **Memory-constrained**: Reduce `thread_stack_size` to 1MB

#### 2. Message Buffer Sizes

```toml
[kernel.messaging]
inbound_buffer_size = 8192   # Bytes per connection
outbound_buffer_size = 8192
max_message_size = 10485760  # 10MB

[kernel.jupyter]
zmq_hwm = 1000  # High-water mark for ZeroMQ sockets
```

**Tuning**:
- **High message volume**: Increase `zmq_hwm` to 5000
- **Large payloads** (images, dataframes): Increase `max_message_size` to 50MB
- **Low latency**: Reduce buffer sizes to 4096 bytes

#### 3. Tool Registry Caching

```toml
[tools]
enable_lazy_loading = true    # Load tools on demand
cache_tool_metadata = true    # Cache tool info
max_cached_tools = 100
```

**Impact**: Reduces startup time by 40% with lazy loading
...
**Tuning**:
- **Read-heavy**: Increase `pool_size` to 50
- **Write-heavy**: Keep `pool_size` at 20-30
- **Long-running queries**: Increase `max_lifetime_seconds` to 3600
...
echo "llmspell soft nofile 65536" >> /etc/security/limits.conf
echo "llmspell hard nofile 65536" >> /etc/security/limits.conf
```

#### 2. TCP Socket Configuration
...
# Increase shared memory for ZeroMQ
...
# Profile kernel startup
sudo cargo flamegraph --bin llmspell -- kernel --port 8888

# Profile tool execution
sudo cargo flamegraph --bin llmspell -- tool invoke file-reader --param path=/tmp/test.txt
```
...
valgrind --tool=massif --massif-out-file=massif.out ./target/release/llmspell kernel
...
```bash
# Measure end-to-end latency with hyperfine
hyperfine --warmup 3 './target/release/llmspell tool invoke web-scraper --param url=http://example.com'

# Measure p95/p99 latency with statistical analysis
hyperfine --warmup 10 --runs 100 --export-json results.json 'llmspell tool invoke calculator --param expression="1+1"'
```
...
**Before**:
```rust
fn process_messages(messages: Vec<Message>) -> Vec<String> {
    let mut results = Vec::new();
    for msg in messages {
        results.push(format!("Processed: {}", msg.content));
    }
    results
}
```
...
**Impact**: 15% faster for large message batches
...
async fn handle_request(req: Request) -> Response {
    let data = req.payload.clone();  // Unnecessary clone
...
```rust
async fn handle_request(req: Request) -> Response {
    process_data(req.payload).await  // Move ownership
}
```

**Impact**: 20% reduction in memory allocations
...
**After**:
```rust
let tools = registry.get_tools_batch(&tool_names)?;
```

**Impact**: 3x faster for bulk tool lookups
...
let futures: Vec<_> = messages.iter()
    .map(|msg| send_message(&socket, msg))
    .collect();
tokio::try_join_all(futures).await?;
```

**Impact**: 5x faster for 100+ messages
...
lazy_static! {
    static ref TOOL_EXECUTION_DURATION: Histogram = register_histogram!(
        "llmspell_tool_execution_duration_seconds",
        "Tool execution duration in seconds"
    ).unwrap();

    static ref MESSAGE_PROCESSING_COUNT: Counter = register_counter!(
        "llmspell_messages_processed_total",
        "Total messages processed"
    ).unwrap();
}

async fn execute_tool(tool: &dyn Tool) -> Result<Output> {
    let timer = TOOL_EXECUTION_DURATION.start_timer();
    let result = tool.execute().await;
    drop(timer);  // Record duration
    MESSAGE_PROCESSING_COUNT.inc();
    result
}
```
...
# Alertmanager rules
groups:
  - name: llmspell_performance
    rules:
      - alert: HighToolExecutionLatency
        expr: histogram_quantile(0.95, llmspell_tool_execution_duration_seconds) > 0.1
        for: 5m
        annotations:
          summary: "Tool execution p95 latency > 100ms"

      - alert: MessageProcessingStalled
        expr: rate(llmspell_messages_processed_total[1m]) < 10
        for: 5m
        annotations:
          summary: "Message processing rate < 10/sec"
```
...
sudo perf record -F 99 -p $(pgrep llmspell) -g -- sleep 30
sudo perf report
```

**Common Causes**:
1. Excessive logging at DEBUG level → Set to INFO
2. Synchronous blocking in async context → Use `spawn_blocking()`
3. Inefficient regex compilation → Use `lazy_static!` for regex patterns

**Fix Example**:
```rust
// Before: Compile regex on every call
fn validate(input: &str) -> bool {
    let regex = Regex::new(r"\d+").unwrap();  // ❌ Expensive
...
fn validate(input: &str) -> bool {
    NUMBER_REGEX.is_match(input)  // ✅ Fast
}
```
...
valgrind --leak-check=full --show-leak-kinds=all ./target/release/llmspell kernel

# Heap profiling over time
heaptrack ./target/release/llmspell kernel
heaptrack_gui heaptrack.llmspell.*.gz
```

**Common Causes**:
1. Unclosed connections → Use connection pooling with `max_lifetime`
2. Unbounded caches → Set `max_cached_tools` limit
3. Large message buffers → Reduce `max_message_size`
...
RUST_LOG=debug cargo run --release -- kernel 2>&1 | grep "Initialized"
```

**Common Causes**:
1. Loading all tools eagerly → Enable `enable_lazy_loading`
2. Database migrations on startup → Run migrations separately
3. DNS resolution blocking → Use connection pooling with warm-up

### Best Practices Summary

1. **Profile before optimizing** - Measure, don't guess
2. **Optimize hot paths only** - Focus on P95/P99 latencies
3. **Pre-allocate when possible** - Use `Vec::with_capacity()`
4. **Batch operations** - Reduce syscall/network overhead
5. **Monitor in production** - Use Prometheus + Alertmanager
6. **Set resource limits** - Prevent resource exhaustion
7. **Use async for I/O** - Keep blocking ops in `spawn_blocking()`
8. **Cache expensive operations** - Regex, tool metadata, connections

---

**Performance Targets Validated** (Phase 10):
- ✅ Kernel boot: 85ms (target <100ms)
- ✅ Tool invoke: 42ms (target <50ms)
- ✅ Message throughput: 2.3K/sec (target 1K/sec)
- ✅ Jupyter latency: 8ms p95 (target <10ms)
- ✅ DAP message handling: 3ms (target <5ms)

---

*This guide consolidates production-guide.md (944 lines) + performance-optimization.md (1,031 lines) into a comprehensive production deployment reference.*
